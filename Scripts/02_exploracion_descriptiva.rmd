...
```{r}
books_full <- read_csv("../Input/books_final_integrated.csv")
...
```{r}
summary(books_to_read$n_to_read)
```

```{r}
books_to_read_median <- books_to_read |> 
  filter(n_to_read > 21 & n_to_read < 95)
```

```{r}
ggplot(books_to_read_median, aes(x =  avg_rating, y = n_to_read)) +
  geom_point(alpha = 0.6, color = "#0B5345", size = 3) +
  geom_smooth(method = "lm", color = "#E67E22", linewidth = 1.2, se = TRUE) +
  labs(
    title = "Relación entre valoración promedio y popularidad anticipada",
...
```{r}
books_to_read_max <- books_to_read |> 
  filter(n_to_read > 95)
```

```{r}
ggplot(books_to_read_max, aes(x =  avg_rating, y = n_to_read)) +
  geom_point(alpha = 0.6, color = "#0B5345", size = 3) +
  geom_smooth(method = "lm", color = "#E67E22", linewidth = 1.2, se = TRUE) +
  labs(
    title = "Relación entre valoración promedio y popularidad anticipada",
...
Esta sección examina cómo se distribuyen las calificaciones promedio, el volumen de calificaciones y las reseñas textuales de los libros en Goodreads a partir de tres dimensiones analíticas principales: el género literario, el período de publicación y, de manera indirecta, la autoría. A diferencia de análisis previos centrados en variables individuales, aquí el énfasis está puesto en identificar patrones comparativos e interrelaciones entre estas dimensiones, utilizando la base consolidada `books_full`.

El objetivo es comprender no solo qué libros reciben mejores evaluaciones, sino también cómo varía la intensidad de la interacción lectora, considerando tanto factores estructurales del campo editorial (género y trayectoria temporal) como elementos simbólicos asociados a la visibilidad de los autores en la plataforma.

## 1. Síntesis descriptiva: género y período de publicación
...
- Se ordena a partir del periodo de publicación 
...
tabla_pregunta4_gt <- tabla_pregunta4 %>%
  
  mutate(
    periodo_publicacion = fct_relevel(
      periodo_publicacion,
      "Antes de 1990",
      "1990-2004",
      "2005-2014",
  ...
  arrange(periodo_publicacion, genre_label) %>%
  
  rename(
    "Género principal" = genre_label,
    "Período de publicación" = periodo_publicacion,
    "Valoración promedio" = promedio_rating,
    "Mediana N° de calificaciones" = mediana_ratings,
    "Mediana N° de reseñas" = mediana_reviews,
    "Mediana 'Por leer'" = mediana_to_read,
    "N° de libros" = n_libros
  ) %>%
  
  gt() %>%
  
  fmt_number(
    columns = "Valoración promedio",
    decimals = 2
  ) %>%
  
  fmt_number(
    columns = c(
      "Mediana N° de calificaciones",
      "Mediana N° de reseñas",
      "Mediana 'Por leer'",
      "N° de libros"
    ),
    decimals = 0,
  ...
  tab_header(
    title = md("**Síntesis descriptiva por género y período de publicación**"),
    subtitle = md("Fuente: Dataset Goodbooks-10k (Kaggle) | Elaboración: *Valentina Tesser*")
  ) %>%
  ...
  tab_style(
    style = cell_fill(color = "#FDF7F7"),
    locations = cells_body(rows = seq(1, nrow(tabla_pregunta4), 2))
  )
...
gtsave(
  tabla_pregunta4_gt,
  "Outputs/tabla_pregunta4_gt.png",
  vwidth  = 2500,
  vheight = 1300,
  zoom    = 2
)
...
## 2. Género principal y volumen de calificaciones
...
## 3. Año de publicación y número de reseñas textuales
