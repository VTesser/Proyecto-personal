...
Igual te recomiendo ver bien qué tipo de variables hay. Si bien no las usas todas al final, hay varias variables que "pasan" como numéricas que son, en verdad, números únicos de identificación.

```{r}
books <- books |> 
  mutate(across(c(book_id, goodreads_book_id, best_book_id, work_id,
                  isbn, isbn13), ~ as.character(.)))
```
...
```{r}
goodreads_bases <- c("book_tags", "books", "ratings", "tags", "to_read")

remover_duplicados_env(goodreads_bases)
```
